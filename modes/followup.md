...
> "No active applications to follow up on. Apply to some roles first with `/career-engine` and come back when they're aging."
...
- Suggest the user run `/career-engine contacto {company}` to find the right person first
...
> - Trying a different contact via `/career-engine contacto`
...
**To:** {email or "No contact found — run `/career-engine contacto` first"}
